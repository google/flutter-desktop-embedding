...
namespace {
	...
	public:
		...
		EncodableValue getAll() {
			...
			if (RegQueryInfoKey(m_Key, NULL, NULL, 0, NULL, NULL, NULL, &valueCount, &maxValueNameLen, &maxValueLen, NULL, NULL) != ERROR_SUCCESS) {
				return EncodableValue(EncodableMap());
			}
			...
			std::unique_ptr<char[]> valueName = std::make_unique<char[]>(maxValueNameLen);
			...
			for (unsigned int i = 0; i < valueCount; i++) {
				...
				DWORD type;
				DWORD result = RegEnumValueA(m_Key, i, valueName.get(), &valueNameLen, NULL, &type, (LPBYTE)value.get(), &valueLen);
				if (result != ERROR_SUCCESS) {
					continue;
				}

				if (type == REG_SZ) {
					map.emplace(EncodableValue(valueName.get()), EncodableValue(value.get()));
				}
				...
				if (type == REG_BINARY)	{
					...
					case Double:
						...
						break;
					case Int32:
						map.emplace(EncodableValue(valueName.get()), EncodableValue(re->int32));
						break;
					case Int64:
						map.emplace(EncodableValue(valueName.get()), EncodableValue(re->int64));
						break;
					case StringList: {
						size_t pos = 0;
						...
						while ((pos + 2) < stringListLength) {
							...
							if (pos + len >= stringListLength) {
								OutputDebugString(TEXT("Invalid binary encoding for StringList"));
								break;
							}
		...
		bool setString(const std::string & name, const EncodableValue & value) {
			...
			return RegSetKeyValueA(m_Key, NULL, name.c_str(), REG_SZ, strValue.c_str(), (DWORD)strValue.length()) == ERROR_SUCCESS;
		}
		...
		bool setInt32(const std::string & name, const EncodableValue & value) {
			...
			return RegSetKeyValueA(m_Key, NULL, name.c_str(), REG_BINARY, &re, sizeof(re)) == ERROR_SUCCESS;
		}
		...
		bool setStringList(const std::string & name, const EncodableValue & value) {
			...
			return RegSetKeyValueA(m_Key, NULL, name.c_str(), REG_BINARY, data.get(), len) == ERROR_SUCCESS;
		}
		...
		bool clear() {
			...
			if (RegQueryInfoKey(m_Key, NULL, NULL, 0, NULL, NULL, NULL, &valueCount, &maxValueNameLen, NULL, NULL, NULL) != ERROR_SUCCESS) {				
				return false;
			}
			...
			for (unsigned int i = 0; i < valueCount; i++) {
				DWORD valueNameLen = maxValueNameLen;				
				DWORD type;
				DWORD result = RegEnumValueA(m_Key, i, valueName.get(), &valueNameLen, NULL, &type, NULL, NULL);
				if (result != ERROR_SUCCESS) {					
					continue;
				}				
	...
	private:
		...
		// opens the registry key using the HKCU\Software\Flutter\<executable_name> key.
		bool openKey(HKEY *key, bool rw) {
			...
			if (RegCreateKeyExA(
				...
				rw ? KEY_WRITE | KEY_READ : KEY_READ,
				NULL,
				key,
				&disp
			) != ERROR_SUCCESS)
	...
	void SharedPreferencesPlugin::RegisterWithRegistrar(
		...
		std::unique_ptr<SharedPreferencesPlugin> plugin(new SharedPreferencesPlugin());
